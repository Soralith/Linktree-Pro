...
<div class="admin-layout">
    ...
    <main class="admin-content">
        <div class="admin-header">
            <h2 class="admin-title">Edit Berita</h2>
        </div>
        ...
        <div class="form-card">
            <form action="{{ route('admin.news.update', $news) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <select name="tags[]" class="form-control @error('tags') is-invalid @enderror" multiple>
                            @foreach($tags as $tag)
                                <option value="{{ $tag->id }}" {{ (collect(old('tags', $news->tags->pluck('id')))->contains($tag->id)) ? 'selected' : '' }}>
                                    {{ $tag->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group">
                    ...
                    @if($news->image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $news->image) }}" alt="Current Image" style="max-width: 300px; border-radius: 12px;">
                        </div>
                    @endif
